...
<script>
	window.onload = function(){ 
		document.oneqUpdateForm.oneq_status.value="<%=oneQVo.getOneq_status() %>" 
	}
</script>
<section class="container justify-content-center">
	...
	<br> <br>
<form name="oneqUpdateForm"method="post" action="${pageContext.request.contextPath}/oneqnaUpdateQ.do?oneq_id=<%=oneQVo.getOneq_id()%>">
	<table class="detail_table">
		<tr>
			<th>제목</th>
			<td colspan="3">
				<input type="text" value="<%=oneQVo.getOneq_title()%>"class="table-input" name ="oneq_title" style="width:100%; border: 0;"/>
			</td>
		</tr>
		<tr>
			<th>작성자</th>
			<td colspan="3" name ="mem_id"><%=oneQVo.getMem_id()%></td>
		</tr>
		<tr>
			<th>분류</th>
			<td class="p-10">
		...
		<tr>
			<th>내용</th>
				<td colspan="3" class="detail-content p-10">
					<textarea name="oneq_cont" rows="10" style="width:100%; border: 0; resize:none;"><%=oneQVo.getOneq_cont() %></textarea>
				</td>
		</tr>
	</table>

	<div class="text-right mt-3" style="text-align: center;">
		<label><i class="fa-solid fa-lock"> 비밀글</i><input type="checkbox" name="oneq_sec" checked class="w-btn" style="size:15px; width:15px; height:15px;"></label><br><br>
		<button type="submit" id="" class="w-btn">수정</button>
		<button type="button" id="listBtn" class="w-btn" style="background:gray;" onclick="location.href='${pageContext.request.contextPath}/oneqnaList.do'">취소</button>
	</div>
</form>
